...
namespace HotUpdate.Scripts.Collector.Collects.Move
{
    public class PeriodicMovement : IItemMovement
    {
        ...
        private Vector3 CalculateSquarePath(float time)
        {
            ...
            return side switch
            {
                0 => new Vector3(-_periodicMovementConfig.amplitude + sideLength * progress, 0, -_periodicMovementConfig.amplitude),
                1 => new Vector3(_periodicMovementConfig.amplitude, 0, -_periodicMovementConfig.amplitude + sideLength * progress),
                2 => new Vector3(_periodicMovementConfig.amplitude - sideLength * progress, 0, _periodicMovementConfig.amplitude),
                _ => new Vector3(-_periodicMovementConfig.amplitude, 0, _periodicMovementConfig.amplitude - sideLength * progress)
            };
        }
    
        private Vector3 CalculateWaypointPosition()
        {
            if (_periodicMovementConfig.waypoints == null || _periodicMovementConfig.waypoints.Length == 0)
                return _startPosition;
        
            int nextIndex = (_currentWaypointIndex + 1) % _periodicMovementConfig.waypoints.Length;
        
            if (!_periodicMovementConfig.loopWaypoints && nextIndex < _currentWaypointIndex)
                nextIndex = _currentWaypointIndex;
        
            Vector3 currentWaypoint = _periodicMovementConfig.waypoints[_currentWaypointIndex];
            Vector3 nextWaypoint = _periodicMovementConfig.waypoints[nextIndex];
            ...
            if (_waypointProgress >= 1f)
            {
                ...
                if (!_periodicMovementConfig.loopWaypoints && _currentWaypointIndex == _periodicMovementConfig.waypoints.Length - 1)
                {
                    // 到达终点，反向
                    Array.Reverse(_periodicMovementConfig.waypoints);
                    _currentWaypointIndex = 0;
                }
            }
